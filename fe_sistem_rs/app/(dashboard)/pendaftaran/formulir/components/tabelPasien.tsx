...
interface Props {
  data: Pendaftaran[];
  loading: boolean;
  onEdit: (row: Pendaftaran) => void;
  onDelete: (row: Pendaftaran) => void;
}

const TabelPendaftaran = ({ data, loading, onEdit, onDelete }: Props) => {
  return (
    <DataTable value={data} paginator rows={10} loading={loading} size="small" scrollable>
      ...
      <Column field="TANGGALKUNJUNGAN" header="Tgl Kunjungan" body={(row: Pendaftaran) => {
