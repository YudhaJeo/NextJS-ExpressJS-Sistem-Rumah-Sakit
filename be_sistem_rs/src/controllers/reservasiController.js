...
export async function createReservasi(req, res) {
    try {
        const { NIK, IDPOLI, IDDOKTER, TANGGALRESERVASI, JADWALPRAKTEK, STATUS, KETERANGAN } = req.body;

        if (!NIK || !IDPOLI || !IDDOKTER || !TANGGALRESERVASI || !JADWALPRAKTEK || !STATUS) {
            return res.status(400).json({ error: 'Semua field wajib diisi' });
        }

        await ReservasiModel.create({ NIK, IDPOLI, IDDOKTER, TANGGALRESERVASI, JADWALPRAKTEK, STATUS, KETERANGAN });
...
export async function updateReservasi(req, res) {
    try {
        ...
        await ReservasiModel.update(id, { NIK, IDPOLI, IDDOKTER, TANGGALRESERVASI, JADWALPRAKTEK, STATUS, KETERANGAN });
