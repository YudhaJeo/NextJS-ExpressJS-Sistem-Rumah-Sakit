...
const TabelTenagaMedis = ({ data, loading, onEdit, onDelete }) => {
  ...
  const listTemplate = (rows, layout) => {
    if (!rows || rows.length === 0) return <div className="col-12">Tidak ada data</div>;
    return <div className="grid grid-nogutter">{rows.map((r, i) => itemTemplate(r, layout, i))}</div>;
  };
      ...
      >
        {selectedData && (
          <div className="p-3 flex flex-column gap-3">
            ...
            />
            <div><b>Kode:</b> {selectedData.KODETENAGAMEDIS}</div>
            <div><b>Jenis Kelamin:</b> {selectedData.JENISKELAMIN === "L" ? "Laki-laki" : "Perempuan"}</div>
            <div><b>Tempat / Tanggal Lahir:</b> {selectedData.TEMPATLAHIR} / {selectedData.TANGGALLAHIR ? new Date(selectedData.TANGGALLAHIR).toLocaleDateString("id-ID") : "-"}</div>
            <div><b>No HP:</b> {selectedData.NOHP}</div>
            <div><b>Email:</b> {selectedData.EMAIL}</div>
            <div><b>Jenis Tenaga Medis:</b> {selectedData.JENISTENAGAMEDIS}</div>
            <div><b>Spesialisasi:</b> {selectedData.SPESIALISASI}</div>
            <div><b>No STR:</b> {selectedData.NOSTR}</div>
            <div><b>Tgl Exp STR:</b> {selectedData.TGLEXPSTR ? new Date(selectedData.TGLEXPSTR).toLocaleDateString("id-ID") : "-"}</div>
            <div><b>No SIP:</b> {selectedData.NOSIP}</div>
            <div><b>Tgl Exp SIP:</b> {selectedData.TGLEXPSIP ? new Date(selectedData.TGLEXPSIP).toLocaleDateString("id-ID") : "-"}</div>
            <div><b>Unit Kerja:</b> {selectedData.UNITKERJA}</div>
            <div><b>Status Kepegawaian:</b> {selectedData.STATUSKEPEGAWAIAN}</div>
            <div><b>Dokumen Pendukung:</b> {selectedData.DOKUMENPENDUKUNG || "-"}</div>
          </div>
        )}
      </Dialog>
    </div>
