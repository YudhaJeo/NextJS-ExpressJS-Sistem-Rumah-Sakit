...
const Page = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${API_URL}/metodePembayaran`);
  ...
  const handleSubmit = async () => {
    ...
    const url = isEdit
      ? `${API_URL}/metodePembayaran/${form.IDMETODE}`
      : `${API_URL}/metodePembayaran`;
  ...
  const handleDelete = (row) => {
    confirmDialog({
      ...
      accept: async () => {
        try {
          await axios.delete(`${API_URL}/metodePembayaran/${row.IDMETODE}`);
          fetchData();
