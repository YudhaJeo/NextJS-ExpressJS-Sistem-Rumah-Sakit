...
import FilterTanggal from '@/app/components/filterTanggal'; // ⏮️ Tambah FilterTanggal
...
const Page = () => {
  ...
  const [originalData, setOriginalData] = useState([]); // ⏮️ Tambah originalData
  const [loading, setLoading] = useState(false);
  const [dialogVisible, setDialogVisible] = useState(false);
  const [startDate, setStartDate] = useState(null); // ⏮️ Filter tanggal
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setOriginalData(res.data.data); // ⏮️ Simpan aslinya
    } catch (err) {
