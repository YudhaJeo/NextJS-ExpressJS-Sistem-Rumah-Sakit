...
function MonitorAntrian() {
  ...
  useEffect(() => {
    ...
    const startPolling = () => {
      if (!pollingInterval) pollingInterval = setInterval(() => fetchData(false), 2000);
    };
    connectWebSocket();
    return () => {
      ws.current?.close();
  ...
  const getNomorAntrianDipanggil = (namaLoket) => {
    const filtered = antrianList.filter((a) => a.LOKET === namaLoket && a.STATUS === 'Dipanggil').sort((a, b) => b.ID - a.ID);
    return filtered?.[0]?.NO_ANTRIAN || '-';
  };

  const getResponsiveConfig = (screen, count) => {
    const width = screen.width;
    if (width < 768) return { cols: 1, numberSize: '2rem', containerPadding: '1rem' };
    if (width < 1024) return { cols: 2, numberSize: '2.5rem', containerPadding: '1rem' };
    if (width < 1440) return { cols: Math.min(count, 3), numberSize: '3rem', containerPadding: '1rem' };
    return { cols: Math.min(count, 4), numberSize: '3.5rem', containerPadding: '1rem' };
  };
      ...
      <div key={index} className={`col-${12 / config.cols}`}>
        ...
        >
          <div style={{ textAlign: 'center' }}>
            ...
            {hasQueue && (
              <Badge value="Dipanggil" severity="info" className="animate-pulse text-xs mt-2" />
            )}
          </div>
        </Card>
      </div>
    ...
    <div className="h-screen flex flex-col overflow-hidden relative bg-white">
      ...
      {!userHasInteracted && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50">
          <Button label="Mulai Tampilkan Antrian" icon="pi pi-play" className="p-button-warning" onClick={() => setUserHasInteracted(true)} />
        </div>
      )}
      ...
      <div className="text-black px-6 py-4 flex justify-between items-center">
        ...
        <div className="bg-blue-100 py-2 overflow-hidden">
          <marquee behavior="scroll" direction="left" scrollamount="4" className="text-blue-900 font-medium text-sm">
            Harap antri dengan tertib • Gunakan masker • Jaga jarak • Cuci tangan sebelum masuk ruangan • Terima kasih 🙏
          </marquee>
        </div>
        ...
        <div className={`px-[${config.containerPadding}] pt-2 shrink-0`}>
          ...
          <div className="flex justify-center gap-8 text-center">
            <div className="flex flex-col items-center">
              <Tag value={loketList.filter((l) => l.AKTIF !== false).length} severity="success" className="text-base font-bold mb-1 px-3 py-2" />
              <span className="text-sm text-black">Loket Aktif</span>
            </div>
            <div className="flex flex-col items-center">
              <Tag value={antrianList.filter((a) => a.STATUS === 'Dipanggil').length} severity="info" className="text-base font-bold mb-1 px-3 py-2" />
              <span className="text-sm text-black">Antrian Dipanggil</span>
            </div>
            <div className="flex flex-col items-center">
              <Tag value={loketList.length} severity="warning" className="text-base font-bold mb-1 px-3 py-2" />
