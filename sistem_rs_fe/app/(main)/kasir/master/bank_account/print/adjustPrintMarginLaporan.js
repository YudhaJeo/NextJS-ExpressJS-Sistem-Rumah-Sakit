...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'LAPORAN DAFTAR BANK ACCOUNT', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['No', 'Nama Bank', 'No Rekening', 'Atas Nama', 'Cabang', 'Kode Bank', 'Status', 'Keterangan']],
      body: dataBankAccount.map((bac) => [
        bac.IDBANK,
        bac.NAMA_BANK,
        bac.NO_REKENING,
        bac.ATAS_NAMA,
        ...
        bac.KODE_BANK,
        bac.STATUS,
        bac.KETERANGAN && bac.KETERANGAN.trim() !== '' ? bac.KETERANGAN : '-',
      ]),
  ...
  const exportExcel = () => {
    const header = ['No', 'Nama Bank', 'No Rekening', 'Atas Nama', 'Cabang', 'Kode Bank', 'Status', 'Keterangan'];
    const body = dataBankAccount.map((bac) => ({
      'No': bac.IDBANK,
      'Nama Bank': bac.NAMA_BANK,
      'No Rekening': bac.NO_REKENING,
      'Atas Nama': bac.ATAS_NAMA,
      'Cabang': bac.CABANG,
      'Kode Bank': bac.KODE_BANK,
      'Status': bac.STATUS,
      'Keterangan': bac.KETERANGAN && bac.KETERANGAN.trim() !== '' ? bac.KETERANGAN : '-',
    }));

    const ws = XLSX.utils.json_to_sheet(body, { header });
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Bank Account');
    XLSX.writeFile(wb, 'Laporan_Bank_Account.xlsx');
  };
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Laporan_Bank_Account');
