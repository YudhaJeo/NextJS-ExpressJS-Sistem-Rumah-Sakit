...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    autoTable(doc, {
      startY: startY,
      head: [['No Invoice', 'NIK', 'Nama Pasien', 'Asuransi', 'Tgl Invoice', 'Total Tagihan', 'Total Deposit', 'Total Angsuran', 'Sisa Tagihan', 'Status']],
  ...
  const exportExcel = () => {
    ...
    const header = ['No Invoice', 'NIK', 'Nama Pasien', 'Asuransi', 'Tgl Invoice', 'Total Tagihan', 'Total Deposit', 'Total Angsuran', 'Sisa Tagihan', 'Status'];
    const body = dataInvoices.map((inv) => ({
      'No Invoice': inv.NOINVOICE,
      'NIK': inv.NIK,
      'Nama Pasien': inv.NAMAPASIEN,
      'Asuransi': inv.ASURANSI,
      'Tgl Invoice': formatTanggal(inv.TANGGALINVOICE),
      'Total Tagihan': formatRupiah(inv.TOTALTAGIHAN),
      'Total Deposit': formatRupiah(inv.TOTALDEPOSIT),
      'Total Angsuran': formatRupiah(inv.TOTALANGSURAN),
      'Sisa Tagihan': formatRupiah(inv.SISA_TAGIHAN),
      'Status': inv.STATUS,
    }));
