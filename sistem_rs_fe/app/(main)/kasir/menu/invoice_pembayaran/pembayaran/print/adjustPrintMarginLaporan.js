...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'LAPORAN DAFTAR PEMBAYARAN', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['No Pembayaran', 'No Invoice', 'NIK', 'Nama Pasien', 'Asuransi', 'Metode', 'Bank', 'Jumlah Bayar', 'Tanggal Bayar', 'Keterangan']],
      body: dataPembayaran.map((pem) => [
        pem.NOPEMBAYARAN,
        pem.NOINVOICE,
        pem.NIK,
        pem.NAMAPASIEN,
        pem.ASURANSI,
        pem.METODEPEMBAYARAN,
        pem.NAMA_BANK || '-',
        formatRupiah(pem.JUMLAHBAYAR),
        formatTanggal(pem.TANGGALBAYAR),
        pem.KETERANGAN && pem.KETERANGAN.trim() !== '' ? pem.KETERANGAN : '-',
      ]),
  ...
  const exportExcel = () => {
    ...
    const header = ['No Pembayaran', 'No Invoice', 'NIK', 'Nama Pasien', 'Asuransi', 'Metode', 'Bank', 'Jumlah Bayar', 'Tanggal Bayar', 'Keterangan'];
    const body = dataPembayaran.map((pem) => ({
      'No Pembayaran': pem.NOPEMBAYARAN,
      'No Invoice': pem.NOINVOICE,
      'NIK': pem.NIK,
      'Nama Pasien': pem.NAMAPASIEN,
      'Asuransi': pem.ASURANSI,
      'Metode': pem.METODEPEMBAYARAN,
      'Bank': pem.NAMA_BANK || '-',
      'Jumlah Bayar': formatRupiah(pem.JUMLAHBAYAR),
      'Tanggal Bayar': formatTanggal(pem.TANGGALBAYAR),
      'Keterangan': pem.KETERANGAN && pem.KETERANGAN.trim() !== '' ? pem.KETERANGAN : '-',
    }));
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Laporan_Pembayaran');
