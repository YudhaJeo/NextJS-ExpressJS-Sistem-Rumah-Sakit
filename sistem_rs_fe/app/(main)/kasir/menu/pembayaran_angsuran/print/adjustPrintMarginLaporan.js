...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    autoTable(doc, {
      startY: startY,
      head: [['No Angsuran', 'No Invoice', 'NIK', 'Nama Pasien', 'Asuransi', 'Nominal', 'Tanggal Bayar', 'Metode', 'Bank', 'Keterangan']],
      body: dataAngsuran.map((ang) => [
        ang.NOANGSURAN,
        ang.NOINVOICE,
        ang.NIK,
        ang.NAMALENGKAP,
        ang.NAMAASURANSI,
        formatRupiah(ang.NOMINAL),
        formatTanggal(ang.TANGGALBAYAR),
        ang.METODE,
        ang.NAMA_BANK || '-',
        ang.KETERANGAN && ang.KETERANGAN.trim() !== '' ? ang.KETERANGAN : '-',
      ]),
  ...
  const exportExcel = () => {
    ...
    const header = ['No Angsuran', 'No Invoice', 'NIK', 'Nama Pasien', 'Asuransi', 'Nominal', 'Tanggal Bayar', 'Metode', 'Bank', 'Keterangan'];
    const body = dataAngsuran.map((row) => ({
      'No Angsuran': row.NOANGSURAN,
      'No Invoice': row.NOINVOICE,
      'NIK': row.NIK,
      'Nama Pasien': row.NAMALENGKAP,
      'Asuransi': row.NAMAASURANSI,
      'Nominal': formatRupiah(row.NOMINAL),
      'Tanggal Bayar': formatTanggal(row.TANGGALBAYAR),
      'Metode': row.METODE,
      'Bank': row.NAMA_BANK || '-',
      'Keterangan': row.KETERANGAN && row.KETERANGAN.trim() !== '' ? row.KETERANGAN : '-',
    }));
