...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'MASTER LOKET', marginLeft, marginTop, marginRight);
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataLoket);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Loket');
    XLSX.writeFile(wb, 'Master Loket.xlsx');
  };
