...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'MASTER ASURANSI', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['ID', 'Nama Asuransi', 'Keterangan']],
      body: dataAsuransi.map((asuransi) => [
        asuransi.IDASURANSI,
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataAsuransi);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Asuransi');
    XLSX.writeFile(wb, 'Master_Asuransi.xlsx');
  };
