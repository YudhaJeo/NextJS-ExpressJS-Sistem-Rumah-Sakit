...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'MASTER DOKTER', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['ID', 'Nama Dokter', 'Nama Poli', 'Jadwal Praktek']],
      body: dataDokter.map((dokter) => [
        dokter.IDDOKTER,
        dokter.NAMALENGKAP,
        dokter.NAMAPOLI,
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataDokter);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Dokter');
    XLSX.writeFile(wb, 'Master Jadwal Dokter.xlsx');
  };
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Master_Jadwal_Dokter');
