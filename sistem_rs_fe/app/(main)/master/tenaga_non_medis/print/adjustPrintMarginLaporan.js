...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'MASTER TENAGA NON MEDIS', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['ID', 'NIP', 'Nama', 'JK', 'Tempat Lahir', 'Tgl Lahir', 'No. Telepon', 'Email', 'Jenis Tenaga', 'Spesialisasi', 'Unit Kerja', 'Status']],
      body: dataNonmedis.map((nonmedis) => [
        nonmedis.IDTENAGANONMEDIS,
        nonmedis.NIP,
        nonmedis.NAMALENGKAP,
        nonmedis.JENISKELAMIN,
        nonmedis.TEMPATLAHIR,
        nonmedis.TANGGALLAHIR ? new Date(nonmedis.TANGGALLAHIR).toLocaleDateString('id-ID', {
          day: '2-digit', month: '2-digit', year: 'numeric',
        }) : '',
        nonmedis.NOHP,
        nonmedis.EMAIL,
        nonmedis.JENISTENAGANONMEDIS,
        nonmedis.SPESIALISASI,
        nonmedis.UNITKERJA,
        nonmedis.STATUSKEPEGAWAIAN,
      ]),
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataNonmedis);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Non Medis');
    XLSX.writeFile(wb, 'Master Tenaga Non Medis.xlsx');
  };
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Master_Tenaga_Non_Medis');
