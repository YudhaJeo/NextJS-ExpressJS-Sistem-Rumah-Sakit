...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'MASTER BERITA', marginLeft, marginTop, marginRight);

     autoTable(doc, {
      startY: startY,
      head: [['ID', 'Judul', 'Deskripsi Singkat']],
      body: dataBerita.map((b) => [
        b.IDBERITA,
        b.JUDUL,
        b.DESKRIPSISINGKAT,
      ]),
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataBerita);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Berita');
    XLSX.writeFile(wb, 'Master_Berita.xlsx');
  };
