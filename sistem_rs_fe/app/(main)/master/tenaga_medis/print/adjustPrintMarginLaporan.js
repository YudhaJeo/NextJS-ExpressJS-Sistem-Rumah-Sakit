...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'MASTER TENAGA MEDIS', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['ID', 'NIP', 'Nama', 'Jenis Kelamin', 'Tempat Lahir', 'Tgl Lahir', 'No. Telepon', 'Email', 'Jenis Tenaga Medis', 'Spesialisasi', 'No. STR', 'No. SIP', 'Unit Kerja', 'Status']],
      body: dataMedis.map((medis) => [
        medis.IDTENAGAMEDIS,
        medis.NIP,
        medis.NAMALENGKAP,
        medis.JENISKELAMIN,
        medis.TEMPATLAHIR,
        medis.TANGGALLAHIR ? new Date(medis.TANGGALLAHIR).toLocaleDateString('id-ID', {
          day: '2-digit', month: '2-digit', year: 'numeric',
        }) : '',
        medis.NOHP,
        medis.EMAIL,
        medis.JENISTENAGAMEDIS,
        medis.SPESIALISASI,
        medis.NOSTR,
        medis.NOSIP,
        medis.UNITKERJA,
        medis.STATUSKEPEGAWAIAN,
      ]),
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataMedis);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Medis');
    XLSX.writeFile(wb, 'Master Tenaga Medis.xlsx');
  };
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Master_Tenaga_Medis');
