...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'NOTIFIKASI', marginLeft, marginTop, marginRight);
    ...
    autoTable(doc, {
      ...
      body: dataNotifikasi.map((notifikasi) => [
        notifikasi.IDNOTIFIKASI,
        notifikasi.NIK,
        notifikasi.TANGGALRESERVASI,
        notifikasi.JUDUL,
        notifikasi.PESAN,
        notifikasi.NAMAPOLI,
        notifikasi.NAMADOKTER,
        notifikasi.STATUS,
      ]),
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataNotifikasi);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Notifikasi');
    XLSX.writeFile(wb, 'Master_Notifikasi.xlsx');
  };
