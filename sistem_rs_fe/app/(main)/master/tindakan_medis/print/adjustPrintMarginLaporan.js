...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'MASTER TINDAKAN MEDIS', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['ID', 'Nama Tindakan', 'Harga', 'Jenis Rawat', 'Kategori', 'Deskripsi']],
      body: dataTindakan.map((tindakan) => [
        tindakan.IDTINDAKAN,
        tindakan.NAMATINDAKAN,
        tindakan.HARGA ? `Rp ${tindakan.HARGA.toLocaleString('id-ID')}` : '-',
        tindakan.JENISRAWAT,
        tindakan.KATEGORI,
        tindakan.DESKRIPSI || '-',
      ]),
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataTindakan);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Tindakan Medis');
    XLSX.writeFile(wb, 'Master Tindakan Medis.xlsx');
  };
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Master_Tindakan_Medis');
