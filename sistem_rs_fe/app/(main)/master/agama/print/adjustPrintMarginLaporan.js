...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'MASTER AGAMA', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['ID', 'Nama Agama']],
      body: dataAgama.map((agama) => [
        agama.IDAGAMA,
        agama.NAMAAGAMA,
      ]),
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataAgama);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Agama');
    XLSX.writeFile(wb, 'Master_Agama.xlsx');
  };
