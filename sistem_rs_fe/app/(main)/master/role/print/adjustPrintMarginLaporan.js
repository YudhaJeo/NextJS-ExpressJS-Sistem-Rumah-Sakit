...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'MASTER ROLE', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['ID', 'Nama Role', 'Jenis Role', 'Keterangan']],
      body: dataRole.map((role) => [
        role.IDROLE,
        role.NAMAROLE,
        role.JENISROLE,
        role.KETERANGAN,
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataRole);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Role');
    XLSX.writeFile(wb, 'Master Role.xlsx');
  };
