...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'MASTER KRITIK SARAN', marginLeft, marginTop, marginRight);
    ...
    autoTable(doc, {
      startY: startY,
      head: [['NIK', 'Jenis', 'Pesan', 'Tanggal']],
      body: dataKritikSaran.map((ks) => [
        ks.NIK,
        ks.JENIS,
        ks.PESAN,
        formatTanggal(ks.CREATED_AT),
      ]),
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataKritikSaran);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'KritikSaran');
    XLSX.writeFile(wb, 'Master_KritikSaran.xlsx');
  };
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Master_KritikSaran');
