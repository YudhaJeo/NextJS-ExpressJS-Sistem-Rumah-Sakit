...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'Reservasi Rawat Jalan', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['ID', 'NIK', 'Nama Pasien', 'Poli', 'Nama Dokter', 'Tanggal', 'Jam Reservasi', 'Status', 'Keterangan']],
      body: dataReservasi.map((reservasi) => [
        ...
        new Date(reservasi.TANGGALRESERVASI).toLocaleString('id-ID', {
          weekday: 'long',
          day: 'numeric',
          month: 'long',
          year: 'numeric',
  ...
  const exportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Reservasi Rawat Jalan');
    XLSX.writeFile(wb, 'Reservasi Rawat Jalan.xlsx');
  };
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Laporan_Komisi_Dokter');
