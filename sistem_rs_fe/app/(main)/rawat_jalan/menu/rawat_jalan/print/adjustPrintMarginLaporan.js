...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'LAPORAN DATA RAWAT JALAN', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['ID', 'No Rekam Medis','Nama Pasien', 'Tanggal Kunjungan', 'Keluhan', 'Poli', 'Dokter', 'Status Rawat', 'Diagnosa', 'Keterangan', 'Status Kunjungan']],
      body: dataRajal.map((rajal) => [
        rajal.IDRAWATJALAN,
        rajal.NOREKAMMEDIS,
        rajal.NAMALENGKAP,
        new Date(rajal.TANGGALKUNJUNGAN).toLocaleDateString('id-ID', { day: '2-digit', month: '2-digit', year: 'numeric' }),
        rajal.KELUHAN,
        rajal.POLI,
        rajal.NAMADOKTER,
        rajal.STATUSRAWAT,
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataRajal);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Rawat Jalan');
    XLSX.writeFile(wb, 'Data Rawat Jalan.xlsx');
  };
