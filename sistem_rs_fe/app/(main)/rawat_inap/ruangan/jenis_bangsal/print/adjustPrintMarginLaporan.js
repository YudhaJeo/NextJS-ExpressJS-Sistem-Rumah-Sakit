...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'DATA JENIS BANGSAL', marginLeft, marginTop, marginRight);
    ...
    autoTable(doc, {
      ...
      body: data.map((jenis) => [
        jenis.IDJENISBANGSAL,
        jenis.NAMAJENIS,
        jenis.FASILITAS,
        new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(jenis.HARGAPERHARI || 0),
      ]),
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(wb, 'Master_Jenis.xlsx');
  };
