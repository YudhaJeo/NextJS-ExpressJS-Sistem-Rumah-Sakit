...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'DATA RIWAYAT RAWAT INAP', marginLeft, marginTop, marginRight);
    ...
    autoTable(doc, {
      ...
      body: data.map((tagihan) => [
        tagihan.NAMALENGKAP,
        tagihan.NOMORBED,
        formatRupiah(tagihan.TOTALOBAT),
        formatRupiah(tagihan.TOTALALKES),
        formatRupiah(tagihan.TOTALTINDAKAN),
        formatRupiah(tagihan.TOTALKAMAR),
        formatTanggal(tagihan.TANGGALMASUK),
        formatTanggal(tagihan.TANGGALKELUAR),
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(wb, 'Master_Riwayat_Ranap.xlsx');
  };
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Master_Riwayat_Ranap');
