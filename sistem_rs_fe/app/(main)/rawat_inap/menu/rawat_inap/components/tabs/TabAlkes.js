...
const TabAlkes = ({ tenagaMedisOptions, statusRawat }) => {
  ...
  const [allAlkesInap, setAllAlkesInap] = useState([]);
  ...
  const fetchAllAlkesInap = async () => {
    try {
      const res = await axios.get(`${API_URL}/alkes_inap`);
      setAllAlkesInap(res.data.data || []);
    } catch (err) {
      console.error('Gagal ambil seluruh data alkes inap:', err);
    }
  };
  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await axios.post(`${API_URL}/alkes_inap`, payload);
  ...
  const columns = [
    ...
    { label: 'Alkes', accessor: 'NAMAALKES' },
  ...
  return (
    <div className="mt-4">
      ...
      <DataTable value={alkesInapData} paginator rows={10} rowsPerPageOptions={[10, 25, 50, 75, 100, 250, 500, 1000]} loading={loading} sortField="WAKTUPEMBERIAN" sortOrder={-1} scrollable>
      ...
      <AdjustPrintMarginLaporanDinamis
        ...
        title="DATA ALKES INAP"
        columns={columns}
        data={alkesInapData}
        allData={allAlkesInap}
        setPdfUrl={setPdfUrl}
