...
export default function AdjustPrintMarginLaporanDinamis({
  adjustDialog,
  setAdjustDialog,
  title = '',
  columns = [],
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, title, marginLeft, marginTop, marginRight);
    autoTable(doc, {
      startY: startY,
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(
      allData.map(row => {
        const obj = {};
        columns.forEach(col => {
          obj[col.label] = col.formatter ? col.formatter(row[col.accessor], row) : row[col.accessor];
        });
        return obj;
      })
    );
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName(title.replace(/\s+/g, '_'));
