...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(listData, adjustConfig) {
    ...
    const startY = addHeader(doc, 'LAPORAN', marginLeft, marginTop, marginRight)
    ...
    autoTable(doc, {
      startY,
      head: [['#', 'JUDUL', 'Deskripsi', 'URL', 'Tanggal']],
      body: listData.map((row, idx) => [
        idx + 1,
        row.JUDUL || '-',
        row.DESKRIPSISINGKAT || '-',
        row.URL || '-',
  ...
  const handleExportPdf = async () => {
    try {
      ...
      const pdfDataUrl = await exportPDF(data, dataAdjust)
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const exportData = data.map((row) => ({
        ID: row.IDBERITA,
        Judul: row.JUDUL,
        Deskripsi: row.DESKRIPSISINGKAT,
        URL: row.URL,
