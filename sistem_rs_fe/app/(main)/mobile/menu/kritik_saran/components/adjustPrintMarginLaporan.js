...
export default function AdjustPrintMarginLaporan({
  ...
  const addHeader = (doc, title, marginLeft, marginTop, marginRight) => {
    ...
    doc.text(
      'Jl. A. Yani No. 84, Pangongangan, Kec. Manguharjo, Kota Madiun, Jawa Timur',
      pageWidth / 2,
      marginTop + 12,
      { align: 'center' }
    )
  ...
  async function exportPDF(listData, adjustConfig) {
    const doc = new jsPDF({
    ...
    const startY = addHeader(doc, 'LAPORAN KRITIK & SARAN', marginLeft, marginTop, marginRight)
    ...
    autoTable(doc, {
      startY,
      head: [['#', 'NIK', 'Pesan', 'Jenis', 'Tanggal']],
      body: listData.map((row, idx) => [
        idx + 1,
        row.NIK || '-',
        row.PESAN || '-',
        row.JENIS || '-',
  ...
  const handleExportPdf = async () => {
    try {
      ...
      const pdfDataUrl = await exportPDF(dataKritikSaran, dataAdjust)
      setPdfUrl(pdfDataUrl)
      setFileName('Laporan_Kritik_Saran')
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const data = dataKritikSaran.map((row) => ({
        ID: row.IDKRITIKSARAN,
        NIK: row.NIK,
        Pesan: row.PESAN,
        Jenis: row.JENIS,
        'Tanggal Dibuat': new Date(row.CREATED_AT).toLocaleDateString('id-ID')
      }))

      const ws = XLSX.utils.json_to_sheet(data)
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, 'Laporan Kritik Saran')
      XLSX.writeFile(wb, `Laporan_Kritik_Saran.xlsx`)
      setAdjustDialog(false)
    } finally {
  ...
  return (
    <Dialog
      ...
      header="Pengaturan Cetak Laporan Kritik & Saran"
      style={{ width: '50vw' }}
    >
