...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'LAPORAN PENGGUNAAN DEPOSIT', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['No Deposit', 'No Invoice', 'NIK', 'Nama Pasien', 'Tanggal Pemakaian', 'Jumlah Pemakaian']],
      body: dataDepositPenggunaan.map((dep) => [
        dep.NODEPOSIT,
        dep.NOINVOICE,
        dep.NIK,
        dep.NAMAPASIEN,
        formatTanggal(dep.TANGGALPEMAKAIAN),
        formatRupiah(dep.JUMLAH_PEMAKAIAN),
      ]),
  ...
  const exportExcel = () => {
    ...
    const header = ['No Deposit', 'No Invoice', 'NIK', 'Nama Pasien', 'Tanggal Pemakaian', 'Jumlah Pemakaian'];
    const body = dataDepositPenggunaan.map((dep) => ({
      'No Deposit': dep.NODEPOSIT,
      'No Invoice': dep.NOINVOICE,
      'NIK': dep.NIK,
      'Nama Pasien': dep.NAMAPASIEN,
      'Tanggal Pemakaian': formatTanggal(dep.TANGGALPEMAKAIAN),
      'Jumlah Pemakaian': formatRupiah(dep.JUMLAH_PEMAKAIAN),
    }));

    const ws = XLSX.utils.json_to_sheet(body, { header });
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Deposit Penggunaan');
    XLSX.writeFile(wb, 'Laporan_Deposit_Penggunaan.xlsx');
  };
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Laporan_Deposit_Penggunaan');
