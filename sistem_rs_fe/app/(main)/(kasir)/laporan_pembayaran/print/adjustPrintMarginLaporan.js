...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'LAPORAN DAFTAR PEMBAYARAN ANGSURAN', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['No Invoice', 'NIK', 'Nama Pasien', 'Asuransi', 'Tanggal Invoice', 'Total Tagihan', 'Total Deposit', 'Total Angsuran', 'Sisa Tagihan', 'Total Pembayaran', 'Metode', 'Status']],
      body: dataLaporan.map((lap) => [
        lap.NOINVOICE,
        lap.NIK,
        lap.NAMAPASIEN,
        lap.ASURANSI,
        formatRupiah(lap.TOTALTAGIHAN),
        formatRupiah(lap.TOTALDEPOSIT),
        formatRupiah(lap.TOTALANGSURANSI),
        formatRupiah(lap.SISATAGIHAN),
        formatRupiah(lap.TOTALPEMBAYARAN),
        lap.METODE,
        lap.STATUS,
      ]),
  ...
  const exportExcel = () => {
    ...
    const header = ['No Invoice', 'NIK', 'Nama Pasien', 'Asuransi', 'Tanggal Invoice', 'Total Tagihan', 'Total Deposit', 'Total Angsuran', 'Sisa Tagihan', 'Total Pembayaran', 'Metode', 'Status'];
    const body = dataLaporan.map((lap) => ({
      'No Invoice': lap.NOINVOICE,
      'NIK': lap.NIK,
      'Nama Pasien': lap.NAMAPASIEN,
      'Asuransi': lap.ASURANSI,
      'Tanggal Invoice': formatTanggal(lap.TANGGalinvoice),
      'Total Tagihan': formatRupiah(lap.TOTALTAGIHAN),
      'Total Deposit': formatRupiah(lap.TOTALDEPOSIT),
      'Total Angsuran': formatRupiah(lap.TOTALANGSURANSI),
      'Sisa Tagihan': formatRupiah(lap.SISATAGIHAN),
      'Total Pembayaran': formatRupiah(lap.TOTALPEMBAYARAN),
      'Metode': lap.METODE,
      'Status': lap.STATUS,
    }));
    ...
    XLSX.writeFile(wb, 'Laporan_Lap_Pembayaran.xlsx');
  };
  ...
  const handleExportPdf = async () => {
    try {
      ...
      setFileName('Laporan_Lap_Pembayaran');
