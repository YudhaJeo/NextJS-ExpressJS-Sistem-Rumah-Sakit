...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'RESERVASI PASIEN', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['ID', 'NIK', 'Nama Lengkap', 'Poli', 'Dokter', 'Tanggal Reservasi', 'Jadwal Praktek', 'Keterangan','Status']],
      body: dataReservasi.map((reservasi) => [
        reservasi.IDRESERVASI,
        reservasi.NIK,
        reservasi.NAMALENGKAP,
        reservasi.NAMAPOLI,
        reservasi.NAMADOKTER,
        reservasi.TANGGALRESERVASI ? new Date(reservasi.TANGGALRESERVASI).toLocaleDateString('id-ID', {
          day: '2-digit', month: '2-digit', year: 'numeric',
        }) : '-',
        reservasi.JAMRESERVASI || reservasi.JADWALPRAKTEK,
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataReservasi);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Reservasi');
    XLSX.writeFile(wb, 'Reservasi Pasien.xlsx');
  };
