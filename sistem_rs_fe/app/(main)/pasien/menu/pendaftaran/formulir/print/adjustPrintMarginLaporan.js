...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'PENDAFTARAN PASIEN', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['ID', 'NIK', 'Nama Lengkap', 'Tanggal Kunjungan', 'Keluhan', 'Poli', 'Status Kunjungan']],
      body: dataFormulir.map((formulir) => [
        formulir.IDPENDAFTARAN,
        formulir.NIK,
        formulir.NAMALENGKAP,
        formulir.TANGGALKUNJUNGAN ? new Date(formulir.TANGGALKUNJUNGAN).toLocaleDateString('id-ID', {
          day: '2-digit', month: '2-digit', year: 'numeric',
        }) : '-',
        formulir.KELUHAN || '-',
        formulir.POLI,
        formulir.STATUSKUNJUNGAN,
      ]),
