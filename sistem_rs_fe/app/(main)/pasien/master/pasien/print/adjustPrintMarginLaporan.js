...
export default function AdjustPrintMarginLaporan({
  ...
  async function exportPDF(adjustConfig) {
    ...
    const startY = addHeader(doc, 'MASTER PASIEN', marginLeft, marginTop, marginRight);

    autoTable(doc, {
      startY: startY,
      head: [['ID', 'No RM', 'NIK', 'Nama Lengkap', 'Tanggal Lahir', 'JK', 'Alamat', 'No HP', 'Usia', 'Agama', 'Gol Darah', 'Asuransi', 'No Asuransi']],
      body: dataPasien.map((pasien) => [
        pasien.IDPASIEN,
        pasien.NOREKAMMEDIS,
        pasien.NIK,
        pasien.NAMALENGKAP,
  ...
  const exportExcel = () => {
    const ws = XLSX.utils.json_to_sheet(dataPasien);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Pasien');
    XLSX.writeFile(wb, 'Master Pasien.xlsx');
  };
